...
<body>
    ...
    <script>
        ...
        productImageInput.addEventListener('change', (e) => {
            ...
            if (file) {
                ...
                if (file.size > maxSize) {
                    showToast('File is too large. Maximum size is 5MB. Your file is ' + (file.size / (1024 * 1024)).toFixed(2) + 'MB', 'error');
                    e.target.value = '';
                    return;
                }
        ...
        document.getElementById('addProductForm').addEventListener('submit', async (e) => {
            ...
            try {
                console.log('Sending request to save-product.php...');
                ...
                if (result.success) {
                    let message = editingProductId ? 'Product updated successfully!' : 'Product created successfully!';
                    if (result.upload_warning) {
                        message += ' Warning: ' + result.upload_warning;
                        console.error('Upload warning:', result.upload_warning);
                    }
                    showToast(message, result.upload_warning ? 'warning' : 'success');

                    // Redirect back to inventory with cache busting to force refresh
                    setTimeout(() => {
                        window.location.href = 'retailerinventory.php?updated=' + Date.now();
                    }, 1500);
                } else {
                    let errorMsg = 'Error: ' + (result.message || 'Failed to save product');
                    if (result.upload_error) {
                        errorMsg += ' (Upload: ' + result.upload_error + ')';
                    }
