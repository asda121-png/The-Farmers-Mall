...
<div class="flex flex-col min-h-screen">
    ...
    <div id="app" class="flex flex-1">
        ...
        <nav class="w-64 bg-white shadow-xl flex flex-col p-4 space-y-2 flex-shrink-0">
            ...
            <button onclick="renderPage('dashboard')" class="nav-item flex items-center p-3 rounded-xl text-gray-700 hover:bg-green-100 transition duration-150 active-nav-item">
            ...
            <button onclick="renderPage('products')" class="nav-item flex items-center p-3 rounded-xl text-gray-700 hover:bg-green-100 transition duration-150">
            ...
            <button onclick="renderPage('orders')" class="nav-item flex items-center p-3 rounded-xl text-gray-700 hover:bg-green-100 transition duration-150">
            ...
            <button onclick="renderPage('finance')" class="nav-item flex items-center p-3 rounded-xl text-gray-700 hover:bg-green-100 transition duration-150">
            ...
            <!-- Vouchers & Coupons -->
            <button onclick="renderPage('coupons')" class="nav-item flex items-center p-3 rounded-xl text-gray-700 hover:bg-green-100 transition duration-150">
            ...
            <button onclick="renderPage('reviews')" class="nav-item flex items-center p-3 rounded-xl text-gray-700 hover:bg-green-100 transition duration-150">
            ...
            <div class="mt-auto pt-4 border-t border-gray-100">
                <p class="text-xs text-gray-500 mb-1">Your Seller ID:</p>
                <div id="user-id" class="text-xs text-gray-700 font-mono break-all bg-gray-50 p-2 rounded-lg mb-2">Loading...</div>
                <!-- Logout Button -->
                <button onclick="signOutUser()" class="w-full flex items-center justify-center p-2 rounded-xl text-red-600 bg-red-50 hover:bg-red-100 transition duration-150 font-medium">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <!-- Main Content Area -->
            <main id="content" class="p-8 transition-all duration-300">
    ...
    <script type="module">
        ...
        const API_KEY = ""; // Canvas provides the API key at runtime
        ...
        async function initializeFirebase() {
            if (!firebaseConfig) {
                showMessage("Firebase config missing. Running in mock mode.", 'error');
                isAuthReady = true;
                return;
            }
            ...
            try {
                ...
                onAuthStateChanged(auth, (user) => {
                    if (user) {
                        ...
                        if (!isAuthReady) {
                            isAuthReady = true;
                            startDataListeners(); // Start listening once authenticated
                            renderPage('dashboard');
                        }
                    } else {
                        userId = null;
                        document.getElementById('user-id').textContent = 'Signed Out';
                        isAuthReady = true; // Still ready, but user is logged out (e.g., anonymous)
                    }
                });
                ...
                showMessage("Auth Error: Check console for details.", 'error');
                // Fallback: If auth fails, still allow the app to initialize, but data won't work
                isAuthReady = true;
                renderPage('dashboard');
            }
        }
        ...
        window.signOutUser = async () => {
            ...
            try {
                await signOut(auth);
                showMessage("Signed out successfully. Refresh to sign in again.", 'info');
                document.getElementById('user-id').textContent = 'Signed Out';
                document.getElementById('content').innerHTML = `
                    <div class="text-center p-12 bg-white rounded-xl shadow-lg mt-16">
                        <h2 class="text-3xl font-bold text-gray-800 mb-4">Goodbye!</h2>
                        <p class="text-gray-600">You have been signed out. Please refresh the page to sign in anonymously again.</p>
                        <button onclick="window.location.reload()" class="mt-6 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition duration-150 font-medium">
                            Refresh and Sign In
                        </button>
                    </div>
                `;
                ...
                showMessage("Error during sign out. See console.", 'error');
            }
        };

        /** Constructs the private user-scoped collection path */
        const getCollectionPath = (collectionName) => {
            if (!userId) {
                throw new Error("User ID is not defined. Authentication is not complete.");
            }
        ...
        function startDataListeners() {
            ...
            onSnapshot(collection(db, getCollectionPath('products')), (snapshot) => {
                ...
                if (document.getElementById('products-list')) {
                    renderProductList(); // Update if on the products page
                }
                if (document.getElementById('dashboard-inventory-alerts')) {
                    renderDashboard(); // Update if on the dashboard
                }
            }, (error) => {
                console.error("Error fetching products:", error);
                showMessage("Failed to load products. Check permissions.", 'error');
            });
        ...
        // Initialize mock products if running without Firebase
        if (!firebaseConfig) {
            products = [
            ...
            isAuthReady = true;
            document.getElementById('user-id').textContent = 'MOCK_USER_ID';
            renderPage('dashboard');
        }
